...
export const api = {
  ...
  services: {
    list: (params: { page?: number; limit?: number; search?: string } = {}) => request<Paginated<Service>>(`/services?${new URLSearchParams(Object.entries({ page: params.page ?? 1, limit: params.limit ?? 10, ...(params.search ? { search: params.search } : {}) }).map(([k, v]) => [k, String(v)]))}`),
  ...
  monitors: {
    list: (params: { page?: number; limit?: number; serviceId?: string; status?: 'UP' | 'DOWN' | 'PAUSED'; search?: string } = {}) => request<Paginated<Monitor>>(`/monitors?${new URLSearchParams(Object.entries({ page: params.page ?? 1, limit: params.limit ?? 10, ...(params.serviceId ? { serviceId: params.serviceId } : {}), ...(params.status ? { status: params.status } : {}), ...(params.search ? { search: params.search } : {}) }).map(([k, v]) => [k, String(v)]))}`),
