...
export const api = {
  ...
  checkResults: { list: (monitorId: string, limit = 50) => request<CheckResult[]>(`/check-results/monitor/${monitorId}?limit=${limit}`), stats: (monitorId: string) => request<MonitorStats>(`/check-results/monitor/${monitorId}/stats`) }
};
