...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try { response = await fetch(`${API_URL}${path}`, { ...options, headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, ...(options.headers ?? {}) }, cache: 'no-store' }); }
  catch { throw new Error("Impossible de joindre l'API. Vérifiez que le backend est disponible et que l'URL API est correctement configurée."); }
  if (!response.ok) { let message = `Erreur API (${response.status})`; try { const body = await response.json(); if (Array.isArray(body.message)) message = body.message.join(', '); else if (body.message) message = body.message; } catch {} throw new Error(message); }
  if (response.status === 204) return undefined as T;
  return response.json();
}

const query = (params: Record<string, string | number | undefined>) => { const clean = Object.entries(params).filter(([, value]) => value !== undefined); return clean.length ? `?${new URLSearchParams(clean.map(([key, value]) => [key, String(value)]))}` : ''; };
