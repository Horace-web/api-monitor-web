...
export default function DashboardPage() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setServiceMeta(serviceData.meta);
      setServiceOptions(optionData.data);
      setMonitors(monitorData.data);
      setMonitorMeta(monitorData.meta);
      setDashboardStats(globalStats);
      ...
      entries.forEach(([id, monitorStats, monitorResults]) => { if (monitorStats) nextStats[id] = monitorStats; nextResults[id] = monitorResults; });
      setStats(nextStats);
  ...
  async function toggleMonitor(monitor: Monitor) {
    try { if (monitor.isActive) await api.monitors.deactivate(monitor.id); else await api.monitors.activate(monitor.id); await loadData(true); }
  ...
  async function deleteMonitor(monitor: Monitor) {
    ...
    try { await api.monitors.remove(monitor.id); await loadData(true); } catch (err) { setError(err instanceof Error ? err.message : 'Suppression impossible.'); }
  }

  async function deleteService(service: Service) {
    if (!window.confirm(`Supprimer « ${service.name} » et ses données associées ?`)) return;
    try { await api.services.remove(service.id); setServicePage(1); setMonitorPage(1); await loadData(true); } catch (err) { setError(err instanceof Error ? err.message : 'Suppression impossible.'); }
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--ink)] text-white">
      <header className="sticky top-0 z-20 border-b border-white/[0.08] bg-[var(--ink)]/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8"><div className="data-mono text-sm font-bold">API<span className="text-[var(--electric)]">/</span>MONITOR</div><div className="flex min-w-0 items-center gap-2 sm:gap-4"><span className="hidden max-w-56 truncate text-xs text-white/40 md:block">{email}</span><button onClick={() => loadData()} disabled={refreshing} className="rounded-lg border border-white/10 px-3 py-2 text-xs font-semibold text-white/65 hover:border-white/20 hover:text-white disabled:opacity-40">{refreshing ? 'Actualisation…' : 'Actualiser'}</button><button onClick={handleSignOut} className="rounded-lg border border-white/10 px-3 py-2 text-xs font-semibold text-white/65 hover:border-white/20 hover:text-white">Déconnexion</button></div></div></header>
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
        <div className="mb-7"><p className="data-mono mb-2 text-xs uppercase tracking-[0.18em] text-[var(--electric)]">Dashboard</p><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Votre monitoring</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">Vue globale de la santé de vos services et endpoints.</p></div><span className="data-mono text-xs text-white/30">{refreshing ? 'SYNC…' : 'LIVE'}</span></div></div>
